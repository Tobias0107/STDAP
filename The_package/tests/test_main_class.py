...
class TestMainClass:
    def test_Sim_trans_dist_single(self):
        ...
        sim.Sim_trans_dist_single(0.25, svg=False, bus_network_redesign=True, minimal_move=False, saving_dir="results_test/blank-slate/percentages/25%/")

    def test_sim_trans_sist_multiple(self):
        sim = Simulator(csv, geopackage, store_in_file=True)
        sim.choose_city("Amsterdam")
        sim.Sim_trans_dist_multiple(0, 0.5, 100, svg=False, bus_network_redesign=False, minimal_move=True, saving_dir="results_test/minimal/range/")
